...
Helm Chart는 API 4개와 Worker 5개, DB migration, ALB Ingress, ExternalSecret을 배포합니다. staging Argo CD Application은 수동 동기화로 시작하며 검증 후 자동 동기화를 켭니다. 로컬 서비스 선언은 kind 환경을 위해 `environments/local/kustomize`에 유지합니다.
