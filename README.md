...
```text
main에 포함된 GitHub Release 발행 → 서비스 CI → ECR Git SHA image → repository_dispatch
...
Helm Chart는 API 4개와 Worker 5개, DB migration, ALB Ingress, ExternalSecret을 배포합니다. staging Argo CD Application은 자동 동기화와 self-heal을 사용합니다. 일반 브랜치 push는 테스트만 수행하며, `main`에 포함된 커밋을 대상으로 GitHub Release를 발행했을 때만 staging 이미지를 승격합니다. 서비스 저장소에는 GitOps 저장소로 dispatch할 수 있는 `GITOPS_DISPATCH_TOKEN` Actions secret이 필요합니다. 로컬 서비스 선언은 kind 환경을 위해 `environments/local/kustomize`에 유지합니다.
